...
export default function InventoryListPage() {
    ...
    const openInventoryModal = (inventory, mode) => {
    ...
    const handleInventoryUpdated = (updated) => {
        setInventories((prev) => prev.map((b) => (b.id === updated.id ? { ...b, ...updated } : b)));
    };

    const handleInventoryCreated = (created) => {
            ...
            <div className="h-full flex items-center justify-center py-12">
                <div className="bg-surface-hover rounded-2xl border border-border shadow-(--shadow-elevation-2) p-6 max-w-md animate-fade-in">
                    <div className="flex items-center gap-3 text-text-primary">
                        ...
                        <div>
                            <p className="font-heading">No se pudieron cargar los nombres de inventario</p>
    ...
    const totalPages = Math.max(1, Math.ceil(filteredInventories.length / itemsPerPage));
    const startIdx = currentPage * itemsPerPage;
    const paginatedInventories = filteredInventories.slice(startIdx, startIdx + itemsPerPage);
    ...
    const handleToggle = async (id, newValue) => {
        try {
            ...
            setNotification({
                severity: "error",
                message: "Error al cambiar el estado del nombre de inventario",
            });
        }
    };
        ...
        <div className="h-full text-text-primary flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2">
                ...
                <p className="text-small text-text-secondary">
                    Gestiona los nombres de inventario disponibles para clasificar materiales.
                </p>
            </div>
            ...
            <div className="bg-surface-hover rounded-2xl border border-border shadow-(--shadow-elevation-4) p-6 flex flex-col gap-4 animate-fade-in">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <SearchField
                        placeholder="Buscar nombre de inventario..."
                        variant="outlined"
                        value={searchTerm}
                ...
                {paginatedInventories.length > 0 ? (
                        ...
                        {paginatedInventories.map((inventory, index) => (
                            ...
                            >
                                <div className="flex items-start justify-between gap-2">
                                    <span className="bg-brand-soft rounded-full w-10 h-10 flex items-center justify-center shrink-0">
                                ...
                                <div className="flex flex-col gap-1">
                                    <h3 className="text-medium font-medium text-text-primary truncate" title={inventory.name}>
                                        {inventory.name}
                                    </h3>
                                    <button
                                        type="button"
                                        onClick={() => openInventoryModal(inventory, "view")}
                    ...
                    <div className="bg-surface-hover rounded-2xl border-2 border-dashed border-border-strong py-12 px-6 flex flex-col items-center gap-3">
                        <span className="bg-surface-hover rounded-full w-14 h-14 flex items-center justify-center shadow-(--shadow-elevation-1)">
                            <Archive size={22} className="text-text-primary" />
                        </span>
                        <p className="text-medium font-medium text-text-primary">
                            {inventories.length === 0
                                ? "Aun no hay nombres de inventario registrados."
                                : "No se encontraron nombres de inventario."}
                        </p>
                        <p className="text-small text-text-secondary text-center">
                            {inventories.length === 0
                                ? "Cuando registres un nombre de inventario, aparecera aqui."
